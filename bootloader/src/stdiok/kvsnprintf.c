...
static inline void format_number_back_buffer(
    ...
    if ((INT128)back_length < (INT128)minimum_field_size) {
        if (left_justify) {
...
static inline void flush_number_format_to_buffer(
    ...
    if (precision > 0 || number != 0) {
        format_number_back_buffer(len, minimum_field_size, leading_zeroes, left_justify, number_buffer, &back_length, back_buffer);
    }
    ...
    est->BootServices->FreePool(number_buffer);
    est->BootServices->FreePool(back_buffer);
...
size_t kvsnprintf(
    ...
    while (*format != u'\0' && written < bufsz) {
        ...
        if (c == u'%') {
            BOOLEAN left_justify = false, force_sign = false, prepend_space = false, alternative_conv = false, leading_zeroes = false;
            ...
            if (c == u'*') {
                ...
                while (c >= u'0' && c <= u'9') {
                    ...
                    minimum_field_size += (INTN)((c - u'9') + 9);
                    c = *format++;
                }
            ...
            switch (c) {
                case u'c': {
                    ...
                    switch (lmodif) {
                        case none:
                            back_buffer = (CHAR16)va_arg(vlist, int);
                            break;
                        case l:
                            back_buffer = (CHAR16)va_arg(vlist, int);
                            break;
                ...
                case u'i': {
                    ...
                    INTN len = kitoa(number, number_buffer, 10);
                ...
                case u'o': {
                    ...
                    INTN len = kutoa(number, number_buffer, 8);
                ...
                case u'X': {
                    ...
                    INTN len = kutoa(number, number_buffer, 16);
                ...
                case u'u': {
                    ...
                    INTN len = kutoa(number, number_buffer, 10);
